...
function validateFavoriteBundle(input) {
  ...
  for (const row of value.entries) {
    ...
    if (object(row) && row.sourceCharacterId === undefined) row.sourceCharacterId = null;
    if (!object(row) || typeof row.id !== 'string' || !row.id || !['tag', 'bundle'].includes(row.kind) || typeof row.seriesId !== 'string' || !(row.sectionId === null || typeof row.sectionId === 'string') || typeof row.title !== 'string' || !validText || typeof row.zh !== 'string' || !Array.isArray(row.aliases) || row.aliases.some(alias => typeof alias !== 'string') || typeof row.note !== 'string' || typeof row.globalSearchable !== 'boolean' || typeof row.pinned !== 'boolean' || typeof row.nsfw !== 'boolean' || !integer(row.order) || !(row.sourceTagId === null || typeof row.sourceTagId === 'string') || !(row.sourceCharacterId === null || typeof row.sourceCharacterId === 'string') || !Number.isFinite(row.createdAt) || !Number.isFinite(row.updatedAt)) return fail('INVALID_ENTRY', '收藏条目无效');
    if (seen.has(row.id)) return fail('DUPLICATE_ID', `重复 ID: ${row.id}`);
    if (!seriesIds.has(row.seriesId)) return fail('SERIES_NOT_FOUND', `系列不存在: ${row.seriesId}`);
