...
function resolveSelection(selection, projection, { includeAdult = false } = {}) {
  ...
  for (const ref of selection) {
    ...
    views.push({ key, kind: ref.kind, displayName: identity.displayName, content: records.map(tag => tag.content).join(', '), adult: records.some(tag => tag.adult), tagIds: records.map(tag => tag.id) });
  }
  return views;
}
module.exports = { selectionKey, resolveSelection };
