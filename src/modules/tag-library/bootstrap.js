...
function createCatalogBootstrap({ userDataDir }) {
  ...
  const library = api.createTagLibrary({ base, repository: api.createLibraryRepository({ filePath: path.join(userDataDir, 'tag-library-v2.json'), backupDir: path.join(userDataDir, 'tag-library-backups') }), legacyInput: () => api.readLegacyInput(storagePath) });
