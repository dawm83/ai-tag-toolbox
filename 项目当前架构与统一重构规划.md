...
更新日期：2026-09-18。代码版本：**V1.4.33 快捷收藏测试版**。本文按当前源码核对，是后续开发的架构入口。

- 工作区：`F:\codex\AI绘画Tag工具箱\ai-tag-release-v143`，本轮提交在 `feat/quick-favorites-v1433`，未推送。
- 桌面测试包：`C:\Users\admin\Desktop\AI绘画Tag工具箱V1.4.33`，以包内 `build-info.json` 的 sourceCommit 区分构建。
...
1. `preload.js` 创建 Storage、Tags、Favorites、Characters、Images、Prompts 和本地识图/翻译适配器。
...
| `src/app-view.js` | 路由、普通 Tag、完整图库/对话、API 表单、识图与子视图组装 | 图库选择和会话 DOM 的唯一所有者；持久状态经模块 API 访问 |
| `src/views/*-view.js` | 角色、Comfy 配置、生图控件、提示词、状态及监视器子视图 | Settings view 只绑定生图三字段；API 表单事件在 app-view；重复的简化图库/会话 factory 已删除 |
...
| `src/modules/bounded-cache.js` | 最近使用优先的有界 Map | Tag/本地翻译各 256 项，单图分析最多 4 个变体；不持久化 |
| `src/modules/tags.js` | 普通词库、分类、搜索、自定义词、手动选择 | 标签素材与 Storage |
| `src/modules/favorites.js` | 收藏系列、子分类、原文条目、搜索、批量管理和独立 Prompt 快照 | Storage；30 步记录差异的历史；按 revision 失效的检索及排列缓存 |
| `src/modules/favorites-transfer.js` | 收藏 JSON 校验、TSV/逐行解析、原文块拼接 | 无 DOM；使用 csv-parse 处理制表符、引号及多行单元格 |
| `src/views/favorites-view.js` | 收藏架、内部搜索、编辑草稿、复制和布局偏好 | 只经 Favorites API 修改业务数据；页面独立绑定、进入、离开及清理 |
...
| 工具名 | 关键输入 | 返回用途 |
| --- | --- | --- |
| `tags.search` | query、category、limit | 普通词释义；角色词另带 attachedData；可发现收藏另带 favorites |
...
会话增量与任务事件使用 250ms 合并保存，结构性修改立即更新存储，成功、错误、取消和超时在返回前等待刷新。窗口关闭前先停止任务并等待保存，不能仅在窗口销毁后的 before-quit 中补写。明确写盘失败时保留窗口并显示通知，下次保存/关闭可重试，不进行后台无限重试；系统强制终止进程仍不保证保存。图片删除/清空同时释放 Blob 与文件，内存 Blob 的异步新增/删除保持顺序。

能力探测由 `assistant.refreshCapabilities` 负责，旧响应不能覆盖较新响应或设置变更后的状态。无调用者的旧 Calls 适配器已移除。工具注册和模型调用协议仍使用现有 primary-tools/Runtime，没有增加权限层。
...
`npm ci` 从项目 lockfile 安装验证依赖。`npm run check` 依次执行 `scripts/check.mjs`、`node --test tests/*.test.cjs`、`tests/regressions-v194.cjs`；check 脚本遍历 main/preload/src/scripts/tests，用 Acorn 检查语法并执行 21 条 ESLint 正确性规则，然后运行已有架构和行为断言。维护构建共 **215 项通过**，含窗口关闭事件契约、持久化/Blob 生命周期、字段隔离、图库批量操作、清空确认和缓存淘汰。不进行风格整改，也不把 Node/jsdom 测试当作 Electron、真实 AI/ComfyUI 或性能实测。
