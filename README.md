...
当前本地开发测试版为 **V1.4.352**：主 AI 直接看图并选择必要工具；本地识图只作初始提示；主 AI 会区分目标图、属性参考图和无关图片；会话重开时会从绘图结果恢复图片关系；修改要求明确指向某张候选图时直接续绘，无法判断时才询问选择。见 [V1.4.352 交付说明](交付说明-V1.4.352.md)。此版本尚未推送或发布，下面的下载仍指向已发布版本。
