...
- [x] 按现有桌面包复用运行库和模型，验证源码、asar、exe、版本及完整性，交付人工验收。打包源码提交 1664b49；交付后的 4197 个文件、3444 个运行依赖文件、22 个模型文件核验通过，见 `docs/qa/agent-routing-v14334-delivery.json`。
...
第一阶段源码及桌面包已交付，功能提交 ac099be，版本/交付提交 1664b49；随后提交只补充本计划的完成勾选和静态核验记录。包内 build-info.json 指向构建时源码提交，不将文档补记算成新的业务构建。

下一步从“图片输入”开始：核对 vision-service、fixed-subagents、visionClient 的真实视觉内容，发送失败时停止并释放本次临时引用。复刻首轮仍是旧行为，尚未改成本地识图直出。本次不请求新的用户授权；按照项目轮次规则先等待这版人工反馈。
