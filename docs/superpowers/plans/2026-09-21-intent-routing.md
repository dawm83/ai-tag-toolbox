...
- [ ] 添加路由和真实 Runtime/Assistant 测试，复现越界工具、只要 Tag 却请求图片、原话被覆盖、短任务成功后重复调用。
- [ ] 实现路由与策略，接入 Assistant/Runtime，并在现有事件列表显示任务类型和收尾状态。
- [ ] 验证中文/英文、否定、混合任务、带图请求、错误恢复、原生工具成对记录及内部绘图不受影响。
- [ ] npm run check 通过后提交功能；更新应用标识为 1.4.334。
