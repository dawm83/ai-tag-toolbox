...
function fixture(options = {}) {
  ...
  const assistantOptions = { storage, images, comfy,
    ...
    primaryGateway: { complete: async messages => messages.filter(row => typeof row.content === 'string').at(-1).role === 'tool' ? { text: 'done' } : { toolCalls: [{ name: 'generation_execute', arguments: { requirements: 'portrait', outputType: 'images', positiveTags: ['portrait'] } }] } },
