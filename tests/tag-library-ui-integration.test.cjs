...
test('unfavoriting in the favorites page clears the home star and location without deleting the shared Tag', async t => {
  const h = await boot(t);
  const count = h.library.listTags({ includeAdult: true, limit: 0 }).total;
  await h.click('[data-tag-favorite="blue_hair"]');
  h.$('[data-location-parent]').value = 'home'; h.$('[data-location-parent]').dispatchEvent(new h.window.Event('change'));
  h.$('[data-location-child]').value = 'daily'; await h.click('[data-location-confirm]');
  assert.equal(h.$('[data-tag-favorite="blue_hair"]').getAttribute('aria-pressed'), 'true');
  const member = h.library.getMemberships('blue_hair')[0];
  assert.equal(h.library.listTags({ includeAdult: true, limit: 0 }).total, count, 'favoriting never creates a second Tag');
  await h.window.App.route('favorites');
  await h.click(`[data-favorite-action="delete-entry"][data-entry-id="${member.id}"]`);
  await h.click('[data-favorite-action="confirm-delete"]');
  assert.equal(h.library.getTag('blue_hair').favorite, false);
  assert.equal(h.tags.get('blue_hair').favorite, false);
  assert.equal(h.library.listTags({ includeAdult: true, limit: 0 }).total, count);
  await h.window.App.route('tags');
  assert.equal(h.$('[data-tag-favorite="blue_hair"]').getAttribute('aria-pressed'), 'false');
  assert.equal(h.$('[data-tag-locations="blue_hair"]'), null);
  assert.equal(h.$('[data-en="blue_hair"] .zh').textContent, '蓝发');
});

test('edits made from favorites refresh the existing home results when returning', async t => {
  const h = await boot(t);
  const member = (await h.favorites.saveEntry({ sourceTagId: 'blue_hair', seriesId: 'home', sectionId: 'daily' })).data;
  await h.window.App.route('favorites');
  await h.click(`[data-favorite-edit="${member.id}"]`);
  h.input('[data-tag-field="content"]', 'azure hair');
  h.input('[data-tag-field="displayName"]', '天蓝色头发');
  h.input('[data-tag-field="aliases"]', 'sky blue hair');
  h.input('[data-tag-field="note"]', '共享备注');
  await h.click('[data-tag-save]');
  assert.equal(h.characters.get('alice').generalTags[0].content, 'azure hair');
  await h.window.App.route('tags');
  assert.equal(h.$('[data-en="blue_hair"] .en').textContent, 'azure hair');
  assert.equal(h.$('[data-en="blue_hair"] .zh').textContent, '天蓝色头发');
  assert.equal(h.$('[data-tag-note="blue_hair"]').textContent, '共享备注');
  h.$('#q').value = 'sky blue hair'; await h.click('#searchBtn');
  assert.ok(h.$('[data-en="blue_hair"]'));
  await h.window.App.route('favorites');
  await h.click(`[data-favorite-edit="${member.id}"]`);
  h.input('[data-tag-field="searchable"]', false); await h.click('[data-tag-save]');
  await h.window.App.route('tags');
  assert.equal(h.$('[data-en="blue_hair"]'), null, 'returning to an old search respects updated eligibility');
  await h.click('#clearQ');
  assert.equal(h.$('[data-en="blue_hair"] .tag-search-disabled').textContent, '不参与搜索');
});

test('home keeps the remaining favorite location and follows undo after removing one of two memberships', async t => {
  const h = await boot(t);
  const second = (await h.favorites.saveSection({ seriesId: 'home', name: '第二组' })).data;
  const first = (await h.favorites.saveEntry({ sourceTagId: 'blue_hair', seriesId: 'home', sectionId: 'daily' })).data;
  const last = (await h.favorites.saveEntry({ sourceTagId: 'blue_hair', seriesId: 'home', sectionId: second.id })).data;
  await h.window.App.route('favorites');
...
test('home editing and favorite blank creation reuse one editor with placement driven by shared favorite state', async t => {
  const h = await boot(t);
  const editor = h.$('[data-tag-editor-overlay]');
  const contentField = h.$('[data-tag-field="content"]');
  const count = h.library.listTags({ includeAdult: true, limit: 0 }).total;
  await h.click('[data-tag-edit="blue_hair"]');
  await h.click('[data-tag-location]');
  assert(h.$('[data-location-parent] option[value="hair"]'));
  assert.equal(h.$('[data-location-parent] option[value="home"]'), null);
  await h.click('[data-location-cancel]'); await h.click('[data-tag-close]');
  await h.click('[data-tag-favorite="blue_hair"]');
  h.$('[data-location-parent]').value = '__create__'; h.$('[data-location-parent]').dispatchEvent(new h.window.Event('change'));
  h.input('[data-location-parent-name]', '新收藏页');
  h.$('[data-location-child]').value = '__create__'; h.$('[data-location-child]').dispatchEvent(new h.window.Event('change'));
  h.input('[data-location-child-name]', '新标签栏'); await h.click('[data-location-confirm]');
  const member = h.library.getMemberships('blue_hair')[0];
  assert.equal(h.library.listTags({ includeAdult: true, limit: 0 }).total, count);
  await h.window.App.route('favorites'); await h.window.App.views.favorites.focusEntry(member.id);
  await h.click(`[data-favorite-edit="${member.id}"]`);
  assert.equal(h.$('[data-tag-editor-overlay]'), editor);
  assert.equal(h.$('[data-tag-field="content"]'), contentField);
  assert.equal(contentField.value, 'blue hair');
  await h.click('[data-tag-location]');
  assert(h.$(`[data-location-child] option[value="${member.groupId}"]`));
  assert.equal(h.$('[data-location-parent] option[value="hair"]'), null);
  await h.click('[data-location-cancel]'); await h.click('[data-tag-close]');
  await h.favorites.deleteEntries([member.id]);
  await h.window.App.route('tags'); await h.click('[data-tag-edit="blue_hair"]');
  await h.click('[data-tag-location]');
  assert(h.$('[data-location-parent] option[value="hair"]'));
  assert.equal(h.$('[data-location-parent] option[value="home"]'), null);
  await h.click('[data-location-cancel]'); await h.click('[data-tag-close]');
  await h.window.App.route('favorites');
  await h.click(`[data-favorite-quick-new][data-section-id="${member.groupId}"]`);
  assert.equal(h.$('[data-tag-editor-overlay]'), editor);
  assert.equal(h.$('[data-tag-field="content"]'), contentField);
  assert.equal(h.$('[data-tag-field="searchable"]').checked, true);
  assert.equal(h.$('[data-tag-field="adult"]').checked, false);
  h.input('[data-tag-field="content"]', 'new shared tag');
  h.input('[data-tag-field="displayName"]', '新共享标签');
  await h.click('[data-tag-save]');
  assert.equal(editor.hidden, true);
  assert.equal(h.library.listTags({ includeAdult: true, limit: 0 }).total, count + 1);
  const added = h.library.search('new shared tag', { precision: 'exact' }).items[0];
  assert.equal(h.favorites.getEntry(added.favoriteLocations[0].membershipId).tagId, added.id);
  await h.window.App.route('tags');
  assert.equal(h.$(`[data-en="${added.id}"] .zh`).textContent, '新共享标签');
  assert.equal(h.$('[data-favorite-editor], [data-favorite-quick-editor]'), null);
});
